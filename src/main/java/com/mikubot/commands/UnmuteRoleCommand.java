...
public class UnmuteRoleCommand {
    ...
    public static void run(SlashCommandInteractionEvent event) {
        ...
        if (muteRole == null) {
            event.reply("❌ There is no **Muted** role in this server.")
                    .setEphemeral(true).queue();
            return;
        }

        if (!target.getRoles().contains(muteRole)) {
            event.reply("❌ This user does not have the **Muted** role.")
                    .setEphemeral(true).queue();
            return;
        }
        ...
        dmEmbed.setDescription("The **Muted** role has been removed and you can now speak again.");
        ...
        embed.setTitle("🔊 User Unmuted (Role)");
